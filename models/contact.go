...
func (contact *Contact) Update(name string, phone string, userid uint) map[string]interface{} {

	if resp, ok := contact.Validate(); !ok {
		return resp
	}

	GetDB().Model(contact).Updates(Contact{Name: name, Phone: phone, UserId: userid})
	resp := u.Message(true, "success")
	resp["contact"] = contact
	return resp
}
